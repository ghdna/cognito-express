...
exports.DEFAULT_TOKEN_EXPIRATION = void 0;
const jsonwebtoken_1 = __importDefault(require("jsonwebtoken"));
const jwk_to_pem_1 = __importDefault(require("jwk-to-pem"));
const node_fetch_1 = __importDefault(require("node-fetch"));
exports.DEFAULT_TOKEN_EXPIRATION = `3600000`;
class CognitoExpress {
    constructor(config) {
        this.pems = {};
        if (!config)
            throw new TypeError('Options not found. Please refer to README for usage example at https://github.com/ghdna/cognito-express');
        validateConfig(config);
        this.userPoolId = config.cognitoUserPoolId;
        this.tokenUse = config.tokenUse;
        this.tokenExpiration = `${config.tokenExpiration}` || exports.DEFAULT_TOKEN_EXPIRATION;
        this.iss = `https://cognito-idp.${config.region}.amazonaws.com/${this.userPoolId}`;
    }
    async init(callback) {
        try {
            const responseObj = await (0, node_fetch_1.default)(`${this.iss}/.well-known/jwks.json`);
    ...
    validate(token, callback) {
        ...
        if (!decodedJwt)
            return callbackElseThrow(new Error(`Not a valid JWT token`), callback);
        ...
        if (!pem)
            return callbackElseThrow(new Error(`Invalid ${this.tokenUse} token`), callback);
        const params = {
            token: token,
            pem: pem,
            iss: this.iss,
...
function jwtVerify(params, callback) {
    jsonwebtoken_1.default.verify(params.token, params.pem, {
        issuer: params.iss,
...
function callbackElseThrow(err, callback) {
    if (callback) {
        callback(err);
    }
    else {
...
exports.default = CognitoExpress;
